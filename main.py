...
def fuzzy_correct(symptom_input):
    """Correct symptom using fuzzy matching if possible."""
    corrected = []
...
# Fuzzy match symptoms against our symptoms_dict
def correct_symptoms(user_symptoms):
    corrected = []
    all_symptoms = list(symptoms_dict.keys())

    for s in user_symptoms:
        match = difflib.get_close_matches(s.lower().replace(" ", "_"), all_symptoms, n=1, cutoff=0.6)
        if match:
